...
import { Plus, Layers, Pencil, Trash2, X, SlidersHorizontal } from 'lucide-react'
import { Badge } from '@/components/ui/badge'
import { FilamentForm } from './filament-form'
import { BatchEntryModal } from './batch-entry-modal'
import { getFilaments, upsertFilament, deleteFilament, getPartners, setFilamentStock, consumeFilamentG } from '@/lib/actions/filaments'
...
type SpoolRow = FilamentSpool & {
  price_usd_raw: number
  code?: string
}

// Map DB row (snake_case) → SpoolRow
function fromRow(row: Record<string, unknown>): SpoolRow {
  return {
    id:            String(row.id),
...
function StockAdjustPanel({ spool, onDone }: { spool: SpoolRow; onDone: () => void }) {
